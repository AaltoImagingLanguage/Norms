function h=som_show_add_scaled(mode,D,varargin)
...
for i=1:2:length(varargin),
  ...
  switch identifier  
   ...
   case 'distances'
    if ~vis_valuetype(value, {'nxm'})
      error('distances needs to be an n_words x n_cells matrix');
    end
    used_in.comet=0;              % Set relevance flags
    used_in.traj=0;
    used_in.label=1;
   ...
   case 'vocab'
    if ~vis_valuetype(value, {'cellcolumn_of_char'})
      error('vocab needs to be an n_words x 1 cell matrix');
    end
    used_in.comet=0;              % Set relevance flags
    used_in.traj=0;
    used_in.label=1;
...
for i=1:length(p);
  ...
  % Write labels
  [S,m,l,t]=som_grid_scaled(lattice, msize, ...
		     'Line','none', ...
		     'Marker', 'none', ...
		     'Label', Labels, ...
		     'LabelColor', Property.textcolor,  ...
