...
def compare_norms(A,B, label_A, label_B,  cats=None):
    ...
    plt.savefig(figure_dir + label_A + "_" + label_B + "pw__norm_comparison.pdf", 
...
plt.savefig(figure_dir + norm1 + "_pairwise_hierarchical_clustering.pdf", 
            format='pdf', dpi=1000, bbox_inches='tight')
norm2_index,norm2_Z = hierarchical_clustering(norm2_vecs.values, 
                                              picks['eng_name'].tolist(),'8.')
plt.savefig(figure_dir + norm2 + "_pairwise_hierarchical_clustering.pdf", 
            format='pdf', dpi=1000, bbox_inches='tight')
